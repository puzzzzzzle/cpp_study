 ...
 * @param aiOutArr
 */
void RandUinqueArrByArrWarrper(int iMiniNum, int iMaxNum, int iArrlen,
                               int aiOutArr[]) {
 ...
 * @param aiOutArr 输出的数组
 */
void RandUinqueArrBySet(int iMiniNum, int iMaxNum, int iArrlen,
                        int aiOutArr[]) {
  assert(iMiniNum >= 0);
  assert(iMiniNum < iMaxNum);
  assert((iMaxNum - iMiniNum) >= iArrlen);
...
int main(int argc, char **argv) {
  ...
  for (unsigned int i = 0; i < sizeof(gap) / sizeof(gap[0]); ++i) {
    for (unsigned int j = 0; j < sizeof(targetLen) / sizeof(targetLen[0]);
         ++j) {
      ...
      std::cout << (endTime.tv_sec - startTime.tv_sec) * 1000000 +
