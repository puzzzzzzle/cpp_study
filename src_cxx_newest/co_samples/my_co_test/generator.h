...
struct generator_send : public generator_base<generator_send<SEND_TYPE>> {
  class promise_type;
  using handle_type = std::coroutine_handle<promise_type>;
  using generator_base<generator_send>::generator_base;

  generator_send(handle_type h) : coro_handle_(h) {}
  ...
  bool send(const SEND_TYPE& send) {
    coro_handle_.promise().send_data_ = send;
    return generator_base<generator_send>::send_base();
  }
  ...
  struct promise_type : public promise_type_base<promise_type> {
    ...
    friend class suspend_always_send_value;
    void return_void() {}
    auto yield_value(int) { return suspend_always_send_value(this); }
    auto get_return_object() {
      auto t = handle_type::from_promise(*this);
      return generator_send{t};
    }
    SEND_TYPE send_data_;
  };
};
