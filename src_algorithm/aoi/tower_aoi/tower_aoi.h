...
class TowerAoiT {
  ...
  typedef void (*ObjCallBackT)(const Object &obj, ObjectType type,
                               const Point &pos,
                               const WatcherTypeObjectMap &appear,
                               const WatcherTypeObjectMap &disAppear,
                               bool IsDiffTower);  // obj变化回调函数
  typedef void (*WatcherCallBackT)(const Watcher &watcher, ObjectType type,
                                   const Point &pos,
                                   const TypeObjectMap &appear,
                                   const TypeObjectMap &disAppear,
...
public:
  ...
  bool MoveObject(const Object &obj, const Point &newPos, ObjectType type) {
    ...
    if (oldTowerIndex.x == newTowerIndex.x &&
      ...
      if (!typeObjectTowerIndexMap[type].erase(obj)) {
        return false;
      }
      if (!m_towers[newTowerIndex.x][newTowerIndex.y]->Add(obj, newPos, type)) {
        return false;
      }
  ...
  bool RemoveWatcher(const Object &watcherObj, ObjectType type) {
    ...
    if (ret) {
      ret = typeWatcherTowerIndexMap[type].erase(watcherObj);
    }
    return ret;
  }
