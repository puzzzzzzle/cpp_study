...
int select_svr(int svr_fd) {
  ...
  char read_buff[MAX_LINE_LEN], write_buff[MAX_LINE_LEN],
      svr_buff[MAX_LINE_LEN], svr_buff1[MAX_LINE_LEN];
      ...
      if (FD_ISSET(client_fd, &curr_set)) {
        if ((read_len = recv(client_fd, read_buff, sizeof(read_buff), 0)) < 0) {
          if (errno == EINTR) {
            ...
            ERROR("something wrong happends when read : "
                  << ":\t" << strerror(errno))
            if (close(client_fd)) {
              ERROR("something wrong happends when close : "
                    << ":\t" << strerror(errno))
            }
            client_fds[i] = -1;
          ...
          write_len =
              do_response(read_buff, read_len, write_buff, sizeof(write_buff));
          if ((real_write_len = (send(client_fd, write_buff, write_len, 0))) !=
              write_len) {
            ERROR("need to send " << write_len << " real send "
                                  << real_write_len)
          }
        }
        if (--ready_number <= 0) {
  ...
  while (is_run) {
    ...
    for (i = 0; i < max_i; i++) {
      ...
      if (all_conns[i].revents & (POLLRDNORM | POLLERR)) {
        if ((read_len =
                 recv(all_conns[i].fd, read_buff, sizeof(read_buff), 0)) < 0) {
          if (errno == EINTR) {
            // sys interrupt
            // do nothing
          } else {
