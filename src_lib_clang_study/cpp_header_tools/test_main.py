...
#     compile_db:cl.CompilationDatabase = cl.CompilationDatabase.fromDirectory(str(win_path))
#     pass


def linux_main():
    build_path = "/tmp/tmp.P2TjzjZZfu"
    cmd_db = Path(f"{build_path}/cmake-build-debug-docker/")
    ...
    src = proc_item["file"]
    out = Path(build_path) / "src_lib_clang_study/generated_test/"
    ch.generated_one(cmd, src, out)
    import clang.cindex as cl
