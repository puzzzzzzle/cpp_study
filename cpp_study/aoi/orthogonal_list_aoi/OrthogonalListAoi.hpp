...
public:
    ...
    virtual std::vector<std::pair<Node *, bool>> QueryInsideRegion(Node *pnode, NumberT xSize, NumberT ySize) override {
    ...
    virtual std::vector<std::pair<Node *, bool>> QueryInsidespace(NumberT xStart, NumberT yStart, NumberT xSize, NumberT ySize) override {
        ...
        while (cur != _tail) {
            if ((abs(cur->point.x - xStart) - cur->point.halfXLen) > xSize) {
                ...
                bool IsOnSide = abs(ySize - cur->point.y) > ySize || abs(xStart - cur->point.x) > xSize;
                if (yDiff <= ySize) {
                    result.push_back({cur, IsOnSide});
                }
            }
