...
int start_svr() {
    ...
    char read_buff[MAX_LINE_LEN], write_buff[MAX_LINE_LEN], svr_buff[MAX_LINE_LEN];
    ssize_t read_len;

    while (is_run) {
        client_addr_len = sizeof(client_addr);
        read_len = recvfrom(svr_fd, read_buff, sizeof(read_buff), 0, (sockaddr *) &client_addr,
                            &client_addr_len);
        ...
        do_response(svr_fd, &client_addr, client_addr_len,
                    read_buff, read_len,
...
void do_response(int fd,
                 sockaddr_in *client_addr, socklen_t len, char *read_buff, ssize_t read_len, char *write_buff,
                 ssize_t write_len) {
