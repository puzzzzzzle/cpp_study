...
void progress_one_request(int fd,
                          const char *__restrict read_buff, size_t read_len,
                          char *write_buff, size_t write_buff_len) {
    int append_len = 5;
    ...
    send(fd, write_buff,
         (read_len + append_len) < write_buff_len ? (read_len + append_len) : write_buff_len, 0);
}
...
int main(int argc, char **argv) {
    ...
    char read_buff[MAX_LINE_LENGTH], write_buff[MAX_LINE_LENGTH], svr_buff[MAX_LINE_LENGTH];
...
#pragma clang diagnostic ignored "-Wmissing-noreturn"
    while (true) {
        ...
        if (poll_conns[0].revents & POLLRDNORM) {
            conn_fd = accept(svr_fd, (sockaddr *) &client_address, &client_address_len);
        ...
        for (i = 0; i < maxi; i++) {
            ...
            if (poll_conns[i].revents & (POLLRDNORM | POLLERR)) {
                if ((read_size = recv(poll_conns[i].fd, read_buff, sizeof(read_buff), 0)) < 0) {
                    if (errno == ECANCELED) {
                        //closed by client
                    ...
                    progress_one_request(poll_conns[i].fd, read_buff, read_size, write_buff, sizeof(write_buff));
                }
                if (--ready_number <= 0) {
                    continue;
                }
