...
public:
     ...
     */
    using ObjCallBackT     = std::function<void(const Object &obj, ObjectType type, const Point &pos,
                                            const WatcherTypeObjectMap &appear, const WatcherTypeObjectMap &disAppear,
                                            bool IsDiffTower)>;  // obj变化回调函数
    using WatcherCallBackT = std::function<void(const Watcher &watcher, ObjectType type, const Point &pos,
                                                const TypeObjectMap &appear, const TypeObjectMap &disAppear,
                                                bool IsDiffTower)>;  // watcher变化回调函数

    using EventCallBackT =
        std::function<void(int EventID, bool isWatcher, const Object &thisObj, const Watcher &thisWatcher, Point point,
                           ObjectType type, const WatcherTypeObjectMap &watchers)>;
    class CallBackDef {
    public:
        enum {
            OBJ_ADD = 0,
        ...
        enum { WATCHER_ADD = 0, WATCHER_REMOVE, WATCHER_MOVE, WATCHER_VIEW_CHANGE, WATCHER_MAX };
    };
...
public:
    ...
    bool RemoveObject(const Object &obj, ObjectType type) {
        ...
        // 删除物体
        auto ret = m_towers[towerIndex.x][towerIndex.y]->Remove(realObjIt->first, type);
        // 删除索引
        if (ret) {
        ...
        if (ret && objectCallBacks[CallBackDef::OBJ_REMOVE]) {
            objectCallBacks[CallBackDef::OBJ_REMOVE](realObjIt->first, type, realObjIt->second, emptyWatchers,
    ...
    bool MoveObject(const Object &obj, const Point &newPos, ObjectType type) {
        ...
        // 拿物体迭代器
        auto &towerObjs = m_towers[oldTowerIndex.x][oldTowerIndex.y]->GetObjs()[type];
        auto  realObjIt = towerObjs.find(obj);
        if (realObjIt == towerObjs.end()) {
            return false;
        }
        ...
        if (oldTowerIndex == newTowerIndex) {
            ...
            if (objectCallBacks[CallBackDef::OBJ_MOVED])
                objectCallBacks[CallBackDef::OBJ_MOVED](realObjIt->first, type, realObjIt->second,
            ...
            // 从老的塔上删掉
            if (!m_towers[oldTowerIndex.x][oldTowerIndex.y]->Remove(realObjIt->first, type)) {
                return false;
            }
            ...
            // 插入新的塔中
            if (!m_towers[newTowerIndex.x][newTowerIndex.y]->Add(realObjIt->first, newPos, type)) {
                return false;
            }
            // 更新索引
    ...
    bool RemoveWatcher(const Object &watcherObj, ObjectType type) {
        ...
        TowerIndexRegion region = GetTowerIndexRegion(towerIndex, realWatcherIt->first.viewRange);
        bool             ret    = false;
        for (int x = region.start.x; x <= region.end.x; ++x) {
            for (int y = region.start.y; y <= region.end.y; ++y) {
                ...
                if (watcherCallBacks[CallBackDef::WATCHER_REMOVE]) {
                    watcherCallBacks[CallBackDef::WATCHER_REMOVE](realWatcherIt->first, type, realWatcherIt->second,
                                                                  emptyObjects, m_towers[x][y]->GetObjs(), false);
                }
            };
        }
        if (ret) {
    ...
    bool MoveWatcher(const Object &watcherObj, const Point &newPos, ObjectType type) {
        ...
        const auto watcher = realWatcherIt->first;  // todo 用引用时 stl把内存写坏了???
    ...
    bool MakeEvent(int eventID, bool isWatcher, const Object &obj, ObjectType type) {
        if (isWatcher) {
            // 拿物体在哪座塔
            TowerIndex oldTowerIndex = GetObjectTowerIndex(obj, type, true);
            if (oldTowerIndex.x < 0) {
            ...
            // 触发事件
            eventCallBack(eventID, isWatcher, emptyObj, realWatcherIt->first, realWatcherIt->second ,type,
                          m_towers[oldTowerIndex.x][oldTowerIndex.y]->GetWatchers());
        } else {
            // 拿物体在哪座塔
            TowerIndex oldTowerIndex = GetObjectTowerIndex(obj, type, true);
            ...
            if(eventCallBack!= nullptr){
                eventCallBack(eventID, isWatcher, realObjIt->first, emptyWatcher, realObjIt->second, type,
                              m_towers[oldTowerIndex.x][oldTowerIndex.y]->GetWatchers());
            }
