...
void no_wait_test() {
  ...
  std::thread t1([m_co_sched]() {
    m_co_sched->Start(0, std::thread::hardware_concurrency());
  });
  t1.detach();
...
void channel_test() {
  ...
  go[=] {
    // Channel是引用计数的, 复制Channel并不会产生新的Channel,
    // 只会引用旧的Channel. 因此, 创建协程时可以直接拷贝Channel.
  ...
  go[=] {
    ...
    // 向ch_1中写入一个数据, 由于ch_1有一个缓冲区空位,
    // 因此可以直接写入而不会阻塞当前协程.
    ch_1 << p1;

    // 再次向ch_1中写入整数2, 由于ch_1缓冲区已满, 因此阻塞当前协程,
    // 等待缓冲区出现空位.
    ch_1 << p1;
  };
  ...
  go[=] {
    ...
    // 由于ch_1在执行前一个协程时被写入了一个元素,
    // 因此下面这个读取数据的操作会立即完成.
    ch_1 >> ptr;

    // 由于ch_1缓冲区已空, 下面这个操作会使当前协程放弃执行权,
    // 等待第一个协程写入数据完成.
    ch_1 >> ptr;
    printf("*ptr = %d\n", *ptr);
  };
