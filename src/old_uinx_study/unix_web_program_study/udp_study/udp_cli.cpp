...
int do_request(const char *msg, ssize_t len, char *receive_buff, size_t buff_len) {
    // init addr
    int         client_fd;
    ...
    if ((send_len = sendto(client_fd, msg, len, 0, (sockaddr *)&svr_addr, sizeof(svr_addr))) != len) {
        ERROR("need to send : " << len << " real send : " << send_len)
    }
    return recvfrom(client_fd, receive_buff, buff_len, 0, (sockaddr *)&svr_addr, &svr_addr_len);
}