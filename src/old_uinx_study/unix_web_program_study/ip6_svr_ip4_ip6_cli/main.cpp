...
TEST(send_testv6_select, hello) {
  ...
  //    ssize_t receive_len = do_request(hello, strlen(hello), receive_buff,
  //    sizeof(receive_buff), hostv4, port_s); ssize_t receive_len =
  //    do_requestv4(hello, strlen(hello), receive_buff, sizeof(receive_buff),
  //    hostv4, port);
  ssize_t receive_len = do_requestv6(hello, strlen(hello), receive_buff,
                                     sizeof(receive_buff), hostv6, port);
  ...
  //            const char *echo_hello = "echo:hello";
  //            char receive_buff[MAX_LINE_LEN];
  //            ssize_t receive_len = do_requestv6(hello, strlen(hello),
  //            receive_buff, sizeof(receive_buff), hostv6,
  //                                               port);
  //
  //            receive_buff[receive_len] = '\0';
  //            EXPECT_EQ(receive_len, strlen(echo_hello));
...
///do_requestv4(hello, strlen(hello), receive_buff, sizeof(receive_buff),
///hostv4, port);
//    ssize_t receive_len = do_requestv4(hello, strlen(hello), receive_buff,
//    sizeof(receive_buff), hostv4, port);
//
//    receive_buff[receive_len] = '\0';
//    EXPECT_EQ(receive_len, strlen(echo_hello));
//    EXPECT_STREQ(echo_hello, receive_buff);
