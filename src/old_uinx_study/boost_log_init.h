...
#define TRACE(msg)                                                   \
  BOOST_LOG_SEV(logger1, trace) << msg << "]   [" << __FILE__ << ":" \
                                << __LINE__ << "    " << __PRETTY_FUNCTION__;
#define DEBUG(msg)                                                   \
  BOOST_LOG_SEV(logger1, debug) << msg << "]   [" << __FILE__ << ":" \
                                << __LINE__ << "    " << __PRETTY_FUNCTION__;
#define INFO(msg)                                                   \
  BOOST_LOG_SEV(logger1, info) << msg << "]   [" << __FILE__ << ":" \
                               << __LINE__ << "    " << __PRETTY_FUNCTION__;
#define WARNING(msg)                                             \
  BOOST_LOG_SEV(logger1, warning)                                \
      << msg << "]   [" << __FILE__ << ":" << __LINE__ << "    " \
      << __PRETTY_FUNCTION__;
#define ERROR(msg)                                                   \
  BOOST_LOG_SEV(logger1, error) << msg << "]   [" << __FILE__ << ":" \
                                << __LINE__ << "    " << __PRETTY_FUNCTION__;
//只是一个参考，调用errno检查错误
#define PERROR(msg)                                                          \
  BOOST_LOG_SEV(logger1, error)                                              \
      << msg << "]   ["                                                      \
