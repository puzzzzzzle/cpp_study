...
static void SplitStdString(const string &src, const string &separator, vector<string> &dest) {
    string            str = src;
    string            substring;
...
eg: tools_conv_arr_bin file1 file2
    ...
    geography.add_options()("cmd,c", opt::value<std::string>()->default_value("show"),
                            R"(process type
show : conv all bin to pic
conv : conv all bin and change first to server type
raw : show origin bin data)");
    geography.add_options()("gap,g", opt::value<int>()->default_value(1), "each grid size of first layer");

    geography.add_options()("grid", opt::value<bool>()->default_value(true)->required(), "need grid and border");
...
eg: --line 100,100:500,500:100,500)");
    geography.add_options()("build-fake", opt::value<int>(), R"(build a fake bin file)");
    geography.add_options()("color", opt::value<std::vector<string>>(), R"(color define: key:value)");
