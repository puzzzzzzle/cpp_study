...
public:
    ...
    static void DrawBorder(Mat& mat, Point start, Point len, const Scalar& s, int bordersize) {
        rectangle(mat, start, len, s, bordersize, LINE_8);
    }
    static void DrawRec(Mat& mat, const Point& start, const Point& offset, const Scalar& s) {
        rectangle(mat, start, Point(start.x + offset.x, start.y + offset.y), s, -1, LINE_8);
    }
