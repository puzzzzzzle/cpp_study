...
TEST(correct_1, toweraoi_obj) {
    ...
    WatcherCallBack watcherCallBack[CallBackDef::WATCHER_MAX]{nullptr, nullptr, nullptr, nullptr};
    ...
    for (int i = 0; i < size; ++i) {
        ...
        objectMap[type][objectBegin + i] = Point(rand() % config.GlobalWith, rand() % config.GlobalHeight);
        ASSERT_TRUE(towerAoi.AddObject(objectBegin + i, objectMap[type][objectBegin + i], type));
    }
    // towerAoi.ToString(std::cout);
...
TEST(correct_1, toweraoi_watcher) {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        watcherMap[type][watcher] = Point(rand() % config.GlobalWith, rand() % config.GlobalHeight);
