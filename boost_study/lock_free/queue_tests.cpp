...
void producer_worker(std::atomic_int *m_producer_count, boost::lockfree::queue<int> *m_queue, int allWorkers)
{
...
void
consumer_worker(std::atomic_int *m_consumer_count, boost::lockfree::queue<int> *m_queue, const std::atomic_bool *done)
{
    int value;
    while (!(*done))
    {
...
void test_impl(const int iterations,
               int producer,
               int consumer)
{
    ...
    { return producer_worker(&m_producer_count, &m_queue, iterations); }; //lambda
    auto consumer_thread_worker = [&]
    { return consumer_worker(&m_consumer_count, &m_queue, &done); };              // equal as bind
